...
PROVENANCE: copied VERBATIM from surplus_value_model/src/pitch_control.py on 2026-07-30
(renamed pitch_control_ref.py). Do not diverge silently — if this file needs changes,
change it upstream and re-copy, or record the divergence here and in NOTES.md.

ROLE IN THIS PROJECT: this is the FROZEN REFERENCE ORACLE for validation gate W1
(docs/validation.md). The concept claims pitch control is the 2D uniform-parameter
special case of the reachable-volume "dome" model. W1 makes that claim testable: the
new src/reach.py, run with uniform parameters in its ground slice, must reproduce this
module's arrival times (tol 1e-9) and control probabilities (tol 1e-6). So this file is
NOT a starting point to edit and grow — it is the thing the new code is measured
against, and it must stay byte-identical to upstream for that comparison to mean
anything. Build the generalization in reach.py; leave this alone.
